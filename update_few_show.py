...
Original file is located at
    https://colab.research.google.com/drive/11HMTzBqT_2-2xZa4JJNZSADwu5vthU_O
"""
...
def train_proto(model, X_train, y_train, epochs=50, n_way=5, k_shot=5, q_query=5):
    optimizer = tf.keras.optimizers.Adam(1e-4)

    for epoch in range(epochs):
        support, query, s_lbl, q_lbl = create_episode(
            X_train, y_train, n_way, k_shot, q_query
        )

        with tf.GradientTape() as tape:
            logits = model.forward_few_shot(
                tf.convert_to_tensor(support),
                tf.convert_to_tensor(query),
                tf.convert_to_tensor(s_lbl),
                n_way
            )
            loss = prototypical_loss(logits, tf.convert_to_tensor(q_lbl))

        grads = tape.gradient(loss, model.trainable_variables)
        optimizer.apply_gradients(zip(grads, model.trainable_variables))

        acc = accuracy(logits, tf.convert_to_tensor(q_lbl))
...
pn_model = PrototypicalNetwork(embedding)

train_proto(pn_model, X_train, y_train)

pn_model.save("model_accent.keras")
...
model = load_model("model_accent.keras", custom_objects=custom_objects)
...
def evaluate_few_shot_model(model, X_test, y_test, n_way=5, k_shot=3, q_query=2, num_episodes=20):
    ...
    for episode in range(num_episodes):
        ...
        # Predict using forward_few_shot
        logits = model.forward_few_shot(
            support_tensor,
            query_tensor,
...
def detect_accent_from_audio(
    ...
    logits = pn_model.forward_few_shot(
        support_tensor,
        query_tensor,
        support_labels_tensor,
        n_way
...
logits_eval = pn_model.forward_few_shot(
    support_tensor_eval,
    query_tensor_eval,
